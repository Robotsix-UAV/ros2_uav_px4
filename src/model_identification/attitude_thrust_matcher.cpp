...
derivative_filter_(30, 1/std::chrono::duration_cast<std::chrono::duration<double>>(sampling_time_).count()),
model_matcher_(1ms, {"limits.max_angle", "limits.min_z_acceleration", "model.vehicle_mass"})
{
    ...
    attitude_setpoint_subscriber_ = this->create_subscription<VehicleAttitudeSetpoint>(
        "fmu/in/vehicle_attitude_setpoint", 1, std::bind(&AttitudeThrustMatcher::attitudeSetpointCallback, this, std::placeholders::_1));
    odometry_subscriber_ = this->create_subscription<VehicleOdometry>(
        "fmu/out/vehicle_odometry", qos, std::bind(&AttitudeThrustMatcher::odometryCallback, this, std::placeholders::_1));
    control_mode_subscriber_ = this->create_subscription<VehicleControlMode>(
        "fmu/out/vehicle_control_mode", qos, std::bind(&AttitudeThrustMatcher::controlModeCallback, this, std::placeholders::_1));
}

void AttitudeThrustMatcher::attitudeSetpointCallback(const VehicleAttitudeSetpoint::SharedPtr attitude_setpoint)
{
...
void AttitudeThrustMatcher::controlModeCallback(const VehicleControlMode::SharedPtr control_mode)
{
    if(status_ == Status::WAITING_DISARM && control_mode->flag_armed == false)
    {
        ...
        auto success = uav_cpp::utils::resampleModuleIO(velocity_data_, resampled_data, sampling_time_, velocity_data_.front().timestamp, velocity_data_.back().timestamp);
        if(!success)
        {
        ...
        // Log the data
        std::vector<std::string> names = {"acceleration_x", "acceleration_y", "acceleration_z", "filtered_acceleration_x", "filtered_acceleration_y", "filtered_acceleration_z"};
        std::vector<std::vector<double>> data;
        for(size_t i = 0; i < derivatives.size(); ++i)
        {
            data.push_back({derivatives[i].x(), derivatives[i].y(), derivatives[i].z(), derivatives_filtered[i].x(), derivatives_filtered[i].y(), derivatives_filtered[i].z()});
        }
        ...
        for(size_t i = 0; i < computed_inputs.size(); ++i)
        {
            ...
            output_data.push_back({computed_inputs[i].acceleration.x(), computed_inputs[i].acceleration.y(), computed_inputs[i].acceleration.z(),
                                 computed_inputs[i].orientation.x(), computed_inputs[i].orientation.y(), computed_inputs[i].orientation.z(), computed_inputs[i].orientation.w(),
                                    resampled_inputs[i].acceleration.x(), resampled_inputs[i].acceleration.y(), resampled_inputs[i].acceleration.z(),
                                    resampled_inputs[i].orientation.x(), resampled_inputs[i].orientation.y(), resampled_inputs[i].orientation.z(), resampled_inputs[i].orientation.w(),
                                    resampled_outputs[i].thrust});
        }
        data_logger_.logToFile("output.csv", timestamps, {"computed_acceleration_x", "computed_acceleration_y", "computed_acceleration_z", "computed_orientation_x", "computed_orientation_y", "computed_orientation_z", "computed_orientation_w",
                                                            "resampled_acceleration_x", "resampled_acceleration_y", "resampled_acceleration_z", "resampled_orientation_x", "resampled_orientation_y", "resampled_orientation_z", "resampled_orientation_w", "resampled_thrust"}, output_data);
    }
}                                                            
}  // namespace ros2_uav::identification