...
public:
    ...
    std::vector<double> computeDerivative() {
        ...
        double dt = 1.0 / fs_;  // Calculate the sampling interval
        for (size_t i = 1; i < data_.size() - 1; ++i) {
            derivatives[i] = (data_[i + 1] - data_[i - 1]) / (2 * dt);  // Use dt in the denominator
        }
        return derivatives;
    }
...
public:
    ...
    void process(std::vector<tf2::Vector3>& derivative, std::vector<tf2::Vector3>& derivative_filtered) {
